...
const ShotContext = Class({
  ...
  initialize: function (panelContext, backend) {
    ...
    this.interactiveWorker = watchWorker(this.tab.attach({
      contentScriptFile: [self.data.url("shooter-interactive-worker.js")]
    }));
    this.interactiveWorker.on("select", (function (info) {
      captureTab(this.tab, info).then(watchFunction(function (imgUrl) {
  ...
  collectInformation: function () {
    // FIXME: chance (maybe high chance) to have concurrency problems here, as the
    // .saves() can overlap.  Maybe we should only save when everything is done.
    watchPromise(captureTab(this.tab, null, {h: 200, w: 350})).then(watchFunction(function (url) {
      console.log("got screenshot", this.id);
    ...
    watchPromise(callScript(
      ...
      {})).then(watchFunction(function (attrs) {
        console.log("got DOM", this.id, Object.keys(attrs));
        this.updateShot(attrs);
      }, this));
  },
...
const Shot = Class({
  ...
  save: function () {
    ...
    if (! metaDone) {
      ...
      console.log("saving to", this.metaUrl, Object.keys(this.dirtyMeta));
