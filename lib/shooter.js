...
function makeShot(backend, autoTag) {
  ...
  var worker = sourceTab.attach({
    contentScriptFile: [self.data.url("make-static-html.js"),
                        ...
                        self.data.url("microformat-shiv.js"),
                        self.data.url("extract.js")]
  });
  var extractedData;
  var framescriptData;
  var screenshot = false;
  captureTab(sourceTab, null, {h: 200, w: 350}).then(function (url) {
    screenshot = url;
    sendData();
  }, function (error) {
  ...
  worker.port.on("data", function (data) {
    extractedData = data;
    extractedData.autoTag = autoTag;
    extractedData.history = processHistory(sourceTab);
    sendData();
  });
  callScript(
    sourceTab,
    self.data.url("make-static-html-framescript.js"),
    "pageshot@documentStaticData",
    {}).then(function (result) {
      framescriptData = result;
      sendData();
    });
  var destWorker;
  ...
  function sendData() {
    if (extractedData && destWorker && destWorkerReady &&
        screenshot !== false && framescriptData) {
      extractedData.screenshot = screenshot;
      for (var attr in framescriptData) {
        extractedData[attr] = framescriptData[attr];
      }
...
function inlineShot(backend, autoTag) {
  ...
  if (inlineShotHistory[sourceUrl]) {
    showPagePanel(panelUrl(inlineShotHistory[sourceUrl]));
    return;
  }
  var worker = sourceTab.attach({
    contentScriptFile: [self.data.url("make-static-html.js"),
  ...
  var screenshot = false;
  var snippet = false;
  captureTab(sourceTab, null, {h: 200, w: 350}).then(function (img) {
    screenshot = img;
    sendData();
  }, function (error) {
    console.error("Error taking shot:", error);
    screenshot = null;
    sendData();
  });
  var metaData = {};
  worker.port.on("requestSnippet", function (info) {
    captureTab(sourceTab, info).then(function (img) {
      metaData.snippet = img;
      worker.port.emit("snippetCaptured", img);
    }, function (error) {
      worker.port.emit("snippetCaptured", null);
    });
  });
  worker.port.on("data", function (data) {
  ...
  function sendData() {
    if ((! extractedData) || screenshot === false) {
      return;
    }
    extractedData.screenshot = screenshot;
...
function showPagePanel(url) {
  ...
  var panel = require("sdk/panel").Panel({
    contentURL: url,
    contentScriptFile: self.data.url("panel-click-watcher.js"),
    onHide: function () {
      tabs.removeListener("ready", onActivate);
    }
  });
  ...
  function onActivate(tab) {
    if (tab.url.indexOf(url) == 0 || url.indexOf(tab.url) == 0) {
      panel.hide();
      panel.destroy();
      tabs.removeListener("ready", onActivate);
    }
  }
  tabs.on("ready", onActivate);
}
...
function panelUrl(url) {
  ...
  var rest = url.substr(url.indexOf("?"));
  if (rest.indexOf("panel") == -1) {
    return url + "&panel";
  }
  return url;
}
