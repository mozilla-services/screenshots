...
function makeShot(backend, autoTag) {
  ...
  worker.port.on("data", function (data) {
    ...
    data.history = sourceTab.history;
    data.openedFromHistory = sourceTab.openedFromHistory;
    extractedData = data;
    sendData();
  });
  var destWorker;
...
function inlineShot(backend, autoTag) {
  ...
  worker.port.on("data", function (data) {
    data.screenshot = screenshot;
    var history = [];
    if (sourceTab.openedFromHistory) {
      sourceTab.openedFromHistory.forEach(function (link) {
        history.push({
          url: link.url,
          title: link.title,
          previousTab: true
    ...
    history = history.concat(sourceTab.history || []);
