...
app.post("/api/update", function(req, res, next) {
  ...
  updateLogin(req.deviceId, { nickname, avatarurl })
    .then(
      ok => {
        if (!ok) {
...
app.post("/api/login", function(req, res) {
  ...
  checkLogin(vars.deviceId, vars.secret, deviceInfo.addonVersion)
    .then(userAbTests => {
      if (userAbTests) {
        let cookies = new Cookies(req, res, { keys: dbschema.getKeygrip() });
        cookies.set("user", vars.deviceId, { signed: true });
...
app.post("/api/delete-shot", function(req, res) {
  ...
  Shot.deleteShot(req.backend, req.body.id, req.deviceId)
    .then(result => {
      if (result) {
        simpleResponse(res, "ok", 200);
      } else {
...
app.post("/api/set-expiration", function(req, res) {
  ...
  Shot.setExpiration(req.backend, shotId, req.deviceId, expiration)
    .then(result => {
      if (result) {
        simpleResponse(res, "ok", 200);
      } else {
...
app.get("/images/:imageid", function(req, res) {
  ...
  Shot.getRawBytesForClip(req.params.imageid).then(obj => {
    if (obj === null) {
      notFound(req, res);
    } else {
...
app.get("/contribute.json", function(req, res) {
  let data = {
    ...
    keywords: ["javascript", "node", "express", "react", "postgresql", "firefox"]
  };
  res.header("Content-Type", "application/json; charset=utf-8");
...
contentApp.get("/content/:id/:domain", function(req, res) {
  let shotId = `${req.params.id}/${req.params.domain}`;
  Shot.getFullShot(req.backend, shotId)
    .then(shot => {
      if (!shot) {
        notFound(req, res);
        return;
