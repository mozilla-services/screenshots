...
const queries = {
  ...
  shotsCreatedByDay: {
    ...
    columns: [{ title: "Number of shots", name: "number_of_shots" }, { title: "Day", type: "date", name: "day" }]
  },
  usersByDay: {
    title: "Users By Day",
    ...
    columns: [{ title: "Number of users", name: "number_of_users" }, { title: "Day", type: "date", name: "day" }]
  },
  shotsByUserHistogram: {
    ...
    columns: [{ title: "Number of users", name: "count" }, { title: "~Number of shots", name: "number_of_shots" }]
  },
  retention: {
...
exports.storeQueries = function() {
  ...
  for (let name in queries) {
    promises.push(
      executeQuery(queries[name]).then(result => {
...
exports.checkLastStoreQueriesTime = function() {
  return db.select(`SELECT created FROM metrics_cache`).then(rows => {
    if (!rows.length) {
      return null;
    }
